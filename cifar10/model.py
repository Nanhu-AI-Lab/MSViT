...
class Token_MS_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, ms_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.q_mp = nn.MaxPool1d(3, stride=2,padding=1, dilation=1)


        self.k_mp = nn.MaxPool1d(3, stride=2,padding=1, dilation=1)
        self.k_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)

        # self.p_mp = nn.MaxPool1d(3, stride=2,padding=1, dilation=1)#nn.AdaptiveMaxPool1d(1)
        ...
        #self.p_conv2 = nn.Conv1d(inter_channels, dim, kernel_size=1, stride=1, bias=False)
        #self.p_bn2 = nn.BatchNorm1d(dim)
        #self.p_lif2 = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
        ...
        #     lif_node_builder(lif_node_type, lif_step_type,v_threshold),
        #     nn.Conv1d(inter_channels, dim, kernel_size=1, stride=1, padding=0),
        #     nn.BatchNorm1d(dim),
        # )

        self.attn_lif = lif_node_builder(lif_node_type, lif_step_type,0.5)#MultiStepParametricLIFNode(
    ...
    def forward(self, x):
        ...
        #q_conv_out = self.q_mp(q_conv_out.flatten(0, 1))
        ...
        #k_conv_out = self.q_mp(k_conv_out.flatten(0, 1))
...
class Spiking_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, ms_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        # self.p_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        # self.p_bn = nn.BatchNorm1d(dim)
        # self.p_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
    ...
    def forward(self, x):
        ...
        # p_conv_out = self.p_conv(x_for_qkv)
        # p_conv_out = self.p_bn(p_conv_out).reshape(T, B, C, N).contiguous()
        # p_conv_out = self.p_lif(p_conv_out)
        # p = p_conv_out.transpose(-1, -2).reshape(T, B, N, self.num_heads, C // self.num_heads).permute(0, 1, 3, 2,
        #                                                                                                4).contiguous()
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        self.mlp1_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)

        self.mlp2_conv = nn.Conv2d(hidden_features, out_features, kernel_size=1, stride=1)
        self.mlp2_bn = nn.BatchNorm2d(out_features)
        self.mlp2_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, ms_scale=None, drop=0., attn_drop=0.,
        ...
        # self.mp =nn.MaxPool1d(3, stride=2,padding=1, dilation=1)#nn.MaxPool1d(4)  # nn.AdaptiveMaxPool1d(1)
        # self.proj_conv = nn.Conv1d(dim, dim*4, kernel_size=1, stride=1)
        # self.proj_bn = nn.BatchNorm1d(dim*4)

    def forward(self, x):
        T, B, C, H, W = x.shape
        x = x + self.tssa(x)#self.mp(x.flatten(3).flatten(0,1)).reshape(T, B, C*2, H, W)
...
class spiking_transformer(nn.Module):
    def __init__(self,
                 ...
                 embed_dims=[64, 128, 256], num_heads=[8, 8, 8], mlp_ratios=[4, 4, 4], qkv_bias=False, ms_scale=None,
...
@register_model
def MSFormer(pretrained=False, **kwargs):
