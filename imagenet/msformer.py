...
__all__ = ['MSFormer_10_512', ]

def compute_non_zero_rate(x):
    if not hasattr(compute_non_zero_rate, "total_neural"):
        compute_non_zero_rate.total_neural = 0  # 初始化静态变量
    if not hasattr(compute_non_zero_rate, "total_fire"):
        compute_non_zero_rate.total_fire = 0  # 初始化静态变量
    x_shape = torch.tensor(list(x.shape))
    compute_non_zero_rate.total_neural = compute_non_zero_rate.total_neural + torch.prod(x_shape)
    compute_non_zero_rate.total_fire = compute_non_zero_rate.total_fire + torch.sum(x)

    print("fire_layer", torch.sum(x), "neural_layer", torch.prod(x_shape), "fire_rate_layer", torch.sum(x) / torch.prod(x_shape), \
          "total fire and neural", compute_non_zero_rate.total_fire,  compute_non_zero_rate.total_neural,\
          "total fire rate", compute_non_zero_rate.total_fire / compute_non_zero_rate.total_neural)



lif_node_type = 'lif'
lif_step_type = 'm'
backend = 'cupy'  # cupy torch
v_threshold=1.0
def lif_node_builder(lif_node_type, lif_step_type,v_threshold):
    if lif_node_type == 'plif':
        return neuron.ParametricLIFNode(step_mode=lif_step_type,
             init_tau=2.0, detach_reset=True, backend=backend,v_threshold=v_threshold)#step_mode=lif_step_type,
    if lif_node_type == 'lif':
        return MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')#neuron.LIFNode(step_mode=lif_step_type, tau=2.0, detach_reset=True, backend=backend,v_threshold=v_threshold)
    if lif_node_type == 'nlif':
        return neuron.LIFNode(step_mode=lif_step_type, tau=2.0, detach_reset=True, backend=backend,v_threshold=v_threshold)
    if lif_node_type == 'glif':
        return neuron.GatedLIFNode(T=4,step_mode=lif_step_type, init_tau=0.25,  backend=backend)



class Token_MS_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, ms_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        super().__init__()
        ...
        self.q_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)

        self.k_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)

        self.p_conv = nn.Conv1d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.p_bn = nn.BatchNorm1d(dim)
        self.p_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)

        self.attn_lif = lif_node_builder(lif_node_type, lif_step_type, 0.5)

        self.proj_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1)
        self.proj_bn = nn.BatchNorm1d(dim)
        self.proj_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
    ...
    def forward(self, x):
        ...
        compute_non_zero_rate(x_for_qkv)
...
class Spiking_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, ms_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.v_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
        self.attn_lif = lif_node_builder(lif_node_type, lif_step_type,0.5)#neuron.ParametricLIFNode(
             #init_tau=2.0, detach_reset=True, backend=backend,v_threshold=0.5)#MultiStepLIFNode(tau=2.0, v_threshold=0.5, detach_reset=True, backend='cupy')
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        self.fc1_lif =  lif_node_builder(lif_node_type, lif_step_type,v_threshold)
        ...
        self.fc2_lif =  lif_node_builder(lif_node_type, lif_step_type,v_threshold)
...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, ms_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.tssa = Token_MS_Attention(dim, num_heads)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, ms_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.attn = Spiking_Self_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, ms_scale=ms_scale,
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj1_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
        ...
        self.proj2_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)

        self.proj_res_conv = nn.Conv2d(embed_dims // 2, embed_dims // 1, kernel_size=1, stride=2, padding=0, bias=False)
        self.proj_res_bn = nn.BatchNorm2d(embed_dims)
        self.proj_res_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
...
class PatchEmbeddingStage(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj3_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
        ...
        self.proj4_lif = lif_node_builder(lif_node_type, lif_step_type,v_threshold)
...
class hierarchical_spiking_transformer(nn.Module):
    def __init__(self,
                 ...
                 embed_dims=[64, 128, 256], num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, ms_scale=None,
        ...
        stage1 = nn.ModuleList([TokenSpikingTransformer(
            ...
            ms_scale=ms_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[j],
        ...
        patch_embed3 = PatchEmbeddingStage3(img_size_h=img_size_h,
                                           img_size_w=img_size_w,
                                           patch_size=patch_size,
                                           in_channels=in_channels,
...
def MSFormer_10_384(T=1, **kwargs):
...
def MSFormer_10_512(T=1, **kwargs):
    model = hierarchical_spiking_transformer(
        T=T,
        img_size_h=224, img_size_w=224,
...
def MSFormer_10_768(T=1, **kwargs):
    model = hierarchical_spiking_transformer(
        T=T,
        img_size_h=224, img_size_w=224,
...
if __name__ == '__main__':
    ...
    model = MSFormer_10_384(T=4).cuda()
